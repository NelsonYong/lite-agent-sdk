...
test("accumulates text deltas and a tool call split across chunks, plus usage", async () => {
  const out = await collect(translateStream(chunks(
    { choices: [{ delta: { content: "He" } }] },
    { choices: [{ delta: { content: "llo" } }] },
    { choices: [{ delta: { tool_calls: [{ index: 0, id: "c1", function: { name: "bash", arguments: "{\"cmd\":" } }] } }] },
    { choices: [{ delta: { tool_calls: [{ index: 0, function: { arguments: "\"ls\"}" } }] } }] },
    { choices: [{ delta: {} }], usage: { prompt_tokens: 7, completion_tokens: 3 } },
  )));
  expect(out.filter((c) => c.type === "text_delta").map((c) => (c as { text: string }).text)).toEqual(["He", "llo"]);
...
test("malformed tool arguments fall back to empty input", async () => {
  const out = await collect(translateStream(chunks(
    { choices: [{ delta: { tool_calls: [{ index: 0, id: "c1", function: { name: "t", arguments: "{bad" } }] } }] },
    { choices: [{ delta: {} }] },
  )));
  const done = out.at(-1) as Extract<ModelChunk, { type: "message_done" }>;
  expect(done.message.content).toEqual([{ type: "tool_call", id: "c1", name: "t", input: {} }]);
});
