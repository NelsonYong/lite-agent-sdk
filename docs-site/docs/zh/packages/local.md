...
lite-agent 的严格单机装配：面向本地模型（Ollama、vLLM、LM Studio、llama.cpp）运行 agent，内置 SQLite 持久化、强制 OS 沙箱、deny-by-default 权限和防篡改审计日志——全部以安全默认值装配到位。
...
`createLocalAgent(config)` 构建于 [`createLiteAgent()`](/zh/packages/sdk) 之上，但锁死了所有安全关键开关。任何会削弱安全姿态的参数都从 `LocalAgentConfig` 中整体移除——你根本无法传入。
...
通过 `tools` 传入的工具必须声明 [`Tool.security`](/zh/packages/core) 元数据，且只接受离线安全的取值——其他情况都会中止启动：
...
- [`@lite-agent/sdk`](/zh/packages/sdk) —— 本包为单机场景加固的通用装配。
- [`@lite-agent/core`](/zh/packages/core) —— provider 无关内核（策略、中间件、事件流）。
- [`@lite-agent/provider`](/zh/packages/provider) —— `localOpenAI` 使用的模型 provider。
- [`@lite-agent/checkpoint-sqlite`](/zh/packages/checkpoint-sqlite) —— 这里使用的 SQLite checkpointer。
- [`@lite-agent/sandbox-anthropic`](/zh/packages/sandbox-anthropic) —— 这里使用的 OS 沙箱运行时。
