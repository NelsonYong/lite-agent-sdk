...
An interactive REPL (`examples/cli`, package `@lite-agent/example-cli`) that wires the **full stack** on top of [`@lite-agent/sdk`](/packages/sdk):

- streaming agent loop via `createLiteAgent`, driven by a provider from [`@lite-agent/provider`](/packages/provider)
- **permission gate** — asks before `bash` / `write_file` / `edit_file`
- **OS-level sandbox** via [`@lite-agent/sandbox-anthropic`](/packages/sandbox-anthropic) — degrades to noop on unsupported environments
