...
Model providers for lite-agent: ready-to-use `ModelProvider` strategies that connect [`@lite-agent/core`](/packages/core) to the Anthropic Messages API and to OpenAI Chat Completions — including OpenAI-compatible and local endpoints.
...
Hand a provider to `query` from [`@lite-agent/sdk`](/packages/sdk) (or to `createAgent` from core):
...
For loopback runtimes, prefer [`@lite-agent/local`](/packages/local): `localOpenAI` ships presets for `ollama`, `vllm`, `lm-studio`, and `llama.cpp`, enforces loopback-only endpoints, and runs a startup health probe.
:::
...
| **Maintained preset** | Repository-owned endpoint preset built on a maintained adapter — the `localOpenAI` presets in [`@lite-agent/local`](/packages/local). Runtime and model capabilities still vary. |
...
- [`@lite-agent/core`](/packages/core) — provider-agnostic agent kernel (strategies, middleware, event stream).
- [`@lite-agent/sdk`](/packages/sdk) — high-level `query` / `createLiteAgent` API.
- [`@lite-agent/local`](/packages/local) — maintained presets for local runtimes (Ollama, vLLM, LM Studio, llama.cpp).
