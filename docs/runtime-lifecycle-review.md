...
本次审查覆盖 SDK、Core、provider、文件/SQLite checkpoint、归档、CLI。当前仓库没有独立的“探索页面”应用源码；页面应消费与 CLI 相同的订阅事件，但本次不能据此声称已经验证该页面。
...
- 普通调用方只需掌握 `send/run`、`subscribe`、`compact`、`listCheckpoints/restore` 和 `close`。页面应只选择一个事件入口渲染，避免同时消费订阅和生成器造成重复。
...
- 独立探索页面仍需要实际源码/URL 才能完成应用层接线与浏览器验收。本仓库已补齐 SDK 事件及 CLI `/compact`、`/checkpoints`、`/restore` 入口。
